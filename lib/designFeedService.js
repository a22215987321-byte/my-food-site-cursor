...
import { wireframeDataUrl } from "./designMockupImage.js";
...
async function writeDailyTraining(db, training) {
  ...
  if (existing.exists) {
    const postId = existing.data()?.postId || null;
    if (await postExists(db, postId)) {
      return { skipped: true, postId };
    }
    await trainingRef.delete().catch(() => {});
  }

  const postRef = db.collection("posts").doc();
  const batch = db.batch();
  batch.set(postRef, postPayload(ART_TEACHER_PROFILE, training.text, {
    isAiDesignPost: true,
...
async function writeDesignRun(db, run, { forceWrite = false } = {}) {
  ...
  if (!forceWrite) {
    ...
    if (existing.exists) {
      const postId = existing.data()?.postId || null;
      if (await postExists(db, postId)) {
        return { skipped: true, postId, runKey: run.slot.slotKey };
      }
  ...
  batch.set(postRef, postPayload(ART_STUDENT_PROFILE, run.studentText, {
    imageUrl: run.mockupImageUrl || wireframeDataUrl(run.target, run.slot),
    isAiDesignPost: true,
    ...
    designMockupEngine: run.mockupImageUrl ? (run.mockupEngine || "wireframe") : "wireframe-inline",
    designAttemptNum: run.attemptNum || 1,
    designTrainingAngle: run.trainingAngle || "",
...
export function shouldTriggerDesignFeed(role, message) {
  ...
  if (role === "artstudent") {
    return {
      trigger: isDesignStudentSubmitCommand(text) || isDesignForceRepostCommand(text),
      forceWrite: isDesignForceRepostCommand(text),
    };
  }
...
export function buildClientPublishPayload(training, run) {
  const studentPost = {
    ...ART_STUDENT_PROFILE,
    text: run.studentText,
    imageUrl: run.mockupImageUrl || wireframeDataUrl(run.target, run.slot),
    videoUrl: null,
    likes: [],
    isAiDesignPost: true,
  ...
  const revisionComment = run.revisionText
    ? {
        ...ART_STUDENT_PROFILE,
        text: run.revisionText,
        isAiDesignRevision: true,
        revisionOfSlotKey: run.slot.slotKey,
      }
    : null;
  ...
  return { studentPost, reviewComment, revisionComment, trainingPost, trainingDateKey: training.dateKey };
}

export function serializeDesignFeedResponse(result) {
  if (!result) return null;
  const posted =
    result.wroteVia === "client_pending"
      ? false
      : result.wroteVia === "admin" && !result.designRun?.skipped && !!result.designRun?.postId;
  return {
    slotKey: result.slotKey,
    reviewStatus: result.reviewStatus,
    targetName: result.run?.target?.name,
    posted,
    wroteVia: result.wroteVia,
    clientPublish: result.clientPublish || null,
...
export function buildDesignTrainingFallbackReply(result, nickname, role = "artteacher") {
  ...
  if (result?.adminError && !result?.clientPublish) {
    return `${prefix}我這邊安排美術生交作品時出了狀況（${result.adminError}），請確認 Firebase Admin 設定，或稍後再試。`;
  }
  if (!result?.run) {
  ...
  const posted = result.wroteVia === "client" || (!designRun?.skipped && !!designRun?.postId);

  if (role === "artstudent") {
    if (!posted && designRun?.skipped) {
...
export async function executeDesignFeedRun({
  force = false,
  preview = false,
  forceWrite = false,
  fromChat = false,
} = {}) {
  const submissionSeq = forceWrite || fromChat ? Date.now() : 0;
  const [training, run] = await Promise.all([
    generateDailyTraining({ force }),
    generateDesignRun({ force: force || forceWrite || fromChat, submissionSeq }),
  ]);
  ...
  const clientPublish = buildClientPublishPayload(training, run);

  // 聊天觸發一律走瀏覽器發佈，不依賴 Firebase Admin（避免 Vercel 憑證問題）
  if (fromChat) {
    return {
      ok: true,
      wroteVia: "client_pending",
      clientPublish,
      training,
      run,
      trainingWrite: null,
      designRun: { skipped: false, postId: null },
      slotKey: run.slot.slotKey,
      reviewStatus: run.reviewStatus,
    };
  }

  const shouldForceWrite = forceWrite;

  try {
    const db = getAdminDb();
    const [trainingWrite, designRun] = await Promise.all([
      writeDailyTraining(db, training),
      writeDesignRun(db, run, { forceWrite: shouldForceWrite }),
    ]);
    ...
    console.error("[design-feed] admin write failed:", adminErr.message);
    return {
      ok: true,
      wroteVia: "client_pending",
