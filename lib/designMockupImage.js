...
async function uploadWireframeMockup(target, slot) {
  ...
  return uploadBufferToR2(buffer, { fileName: "mockup.svg", contentType: "image/svg+xml", prefix: "design-mockups" });
}
...
export async function generateDesignMockupImage({ target, slot, studentText }) {
  try {
    ...
    return { mockupImageUrl: wireframeUrl, mockupEngine: "wireframe" };
  } catch (err) {
    console.error("[design-mockup] upload failed:", err.message);
    return { mockupImageUrl: null, mockupEngine: "none" };
  }
}
